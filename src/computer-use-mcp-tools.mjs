      ...
      properties: {
        action: {
          ...
          properties: {
            ...
            interactionIntent: {
              type: "string",
              enum: ["activate-recognized-text", "activate-control", "select-item"],
              description: "Required for pixel clicks. State the intended UI effect instead of relying on raw geometry. OCR geometry is accepted only for activate-recognized-text. To enter text, do not click first; use atomic coordinate-grounded type_text with screenshot-derived targetBounds.",
            },
            coordinateSpace: {
              type: "string",
